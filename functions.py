        ...
        if accessions[0] and records == 'genome':
            ...
            for feat in rec.features:
                ...
                if feat.type == 'CDS' or feat.type == 'rRNA' or feat.type == 'tRNA':
                    ...
                    try:
                        start.append(feat.location.start)
                    except:
                        start.append('na')

                    try:
                        stop.append(feat.location.end)
                    except:
                        stop.append('na')

                    try:
                        strand.append(feat.strand)
                    except:
                        strand.append('na')

                    try:
                        func.append(feat.qualifiers['product'][0])
                    except:
                        func.append('na')
                    ...
                    try:
                        old_locus_tag.append(
                            feat.qualifiers['old_locus_tag'][0])
                    except:
                        old_locus_tag.append('na')

                    try:
                        gene.append(feat.qualifiers['gene'][0])
                    except:
                        gene.append('na')

                    try:
                        nt.append(str(feat.location.extract(seq)))
                    except:
                        nt.append('na')

                    dna = Seq(str(feat.location.extract(seq)))
                    try:
                        aa.append(str(dna.translate()))
                    except:
                        aa.append('na')
            ...
            plasmid_df2 = pd.DataFrame({'locus_tag': locus_tag, 'old_locus_tag': old_locus_tag, 'gene': gene, 'function': func, 'start': start, 'stop': stop,
                                        'strand': strand, 'nts': nt, 'aa': aa},
                                       columns=['locus_tag', 'old_locus_tag', 'gene', 'function', 'start', 'stop', 'strand', 'nts', 'aa'])
            plasmid_df2['gene_length'] = plasmid_df2['stop'] - \
                plasmid_df2['start']
            print(len(plasmid_df2))
            return plasmid_df2


def get_full_info_df_scaffold(gbk_file_path, records='genome'):
    '''gets all available information pesent in 
    a genbank file depending if that information 
    is there or not
    Parameters:
...
def method_list():
    func_list = ['write_full_seq', 'get_full_info_df', 'write_fna_file', 'write_faa_file', 'method_list',
                 ...
                 'get_codon_adaptation_index','get_codon_frequency', 'get_rbs_sites', 'make_saf_and_concat_genomes','get_upstream_full_info_df', 'make_saf_file_defined_bits']
    for i in func_list:
        print('%s\n' % i)



def get_codon_frequency(gb_path):
    import pandas as pd
    codondict = {}
    codoncount={}
    aas=[]
    codons=[]
    df = get_full_info_df(gbk_file_path=gb_path, records='genome')
    for i,r in df.iterrows():
        aa = r['aa']
        nt = r['nts']
        if len(nt)%3==0:
            amino_acid = [aa[i:i+1] for i in range(0, len(aa), 1)]
            codon     = [nt[i:i+3] for i in range(0, len(nt), 3)]


            if len(amino_acid) == len(codon):
                for i,j in zip(amino_acid,codon):
                    codoncount[j]=codoncount.get(j,0) +1
                    codondict[j]=i
            else:
                continue
    ...
    def get_percent(x):
        return(x/sum(x)*100)
    codon_df = pd.DataFrame({"amino_acid":codondict,"count":codoncount}, columns=['amino_acid','count'])
    codon_df = codon_df.reset_index()
    codon_df.columns = codon_df.columns.str.replace('index','codon')
    codon_df['codon_percent']=codon_df.groupby('amino_acid').transform(get_percent) 
    codon_df = codon_df.sort_values(by=['amino_acid','codon_percent'], ascending=False)
    return codon_df
...
def get_codon_adaptation_index(fasta):
    ...
    for record in SeqIO.parse(fasta, "fasta"):
        ...
        try:
            ...
            gene_ids.append(record.id)
    df = pd.DataFrame({'gene_id': gene_ids, 'cai': index},
                      columns=['gene_id', 'cai'])
    return df
...
def get_rbs_sites(gbk, temp_output, antiSD_seq='ACCTCCTT'):
    ...
    # I use next in order to avoid errors from multiple record genbank files.
    #infile = next(SeqIO.parse(gbk, 'gb'))
        ...
        for feature in rec.features:
            ...
            if feature.type == 'CDS' and 'product' in feature.qualifiers:
                ...
                if feature.strand == 1:
                    ...
                    start.append(st)
                    stop.append(sp)
                    cds_seq.append(str(rec.seq[st:sp]))
                elif feature.strand == -1:
                    real_st.append(feature.location.start.real+1)
                    real_sp.append(feature.location.end.real)
                    strand.append("minus")
    ...
    for index, row in gene_df.iterrows():
        ...
        # this calculates the minimum delta G and the element position of the minimum deltaG
        final_dg = np.min(dg)
        # the element position of the minimum deltaG
        final_element = np.argmin(dg)
...
def get_upstream_full_info_df(gbk_file_path, upstream=25):
    ...
    for feat in rec.features:
        ...
        if feat.type == 'CDS' or feat.type == 'rRNA' or feat.type == 'tRNA':
            ...
            try:
                aa.append(feat.qualifiers['translation'][0])
            ...
            try:
                old_locus_tag.append(feat.qualifiers['old_locus_tag'][0])
...
def make_saf_and_concat_genomes(path,list_name):
    '''function that takes the path and a list that contains
    the names of ffn files. The function makes "psuedo" genome
    which is a result of concatenating all genes into one genome
    string. 
    Finally, all genomes will be written in one fasta file.
    The final fasta file will be used to make bowtie2 indes. 
    This function also makes SAF file which is required for featurecounts
    to determine read counts per each gene per each genome.
    The list file must be in the same directory where the single ffn files exist '''

    sra_df_list=[]
    final_fasta = []
    final_genome_id=[]
    with open('{}/{}'.format(path,list_name), 'r') as f:
        lines = f.readlines()
        for i in lines:
            ...
            seqs = []
            lengths =[]
            for seq_record in SeqIO.parse("{}/{}".format(path,i.strip()),
                                          "fasta"):
                ids.append(seq_record.id)
                seqs.append(str(seq_record.seq))
                lengths.append(len(seq_record))

            genome = ids[0].split('_')[0]
            concated_seq = ''.join(seqs)
            final_fasta.append(concated_seq)
            final_genome_id.append(genome)

            lengths1 = np.array(lengths)
            ends = np.cumsum(lengths1)
            starts = ends - lengths1 + 1

            sra = pd.DataFrame({'GeneID':ids, 'Start':starts, 'End':ends})
            sra['Chr']=genome
            sra['Strand']= '+'

            sra=sra[['GeneID','Chr','Start','End','Strand']]
            sra_df_list.append(sra)


    with open('{}/concated_genomes.ffn'.format(path), 'w+') as f:
        for i,j in zip(final_genome_id,final_fasta):
            f.write('>{}\n{}\n'.format(i,j))

    pd.concat(sra_df_list).to_csv('{}/concated_genomes.saf'.format(path),
                                 sep='\t', index=False)
